...
const Simulator = () => {
    ...
    const handleSimulate = (type) => {
        ...
        const parsedSequence = sequence.split(/[\s,]+/).map(Number);
        const parsedFrames = parseInt(frames);

        if (parsedSequence.some(isNaN) || isNaN(parsedFrames)) {
            alert('Invalid input. Please enter numbers only.');
            return;
        }

        setResult(`Simulating ${type} with ${parsedFrames} frames and sequence [${parsedSequence.join(', ')}]`);
    };
    ...
    return (
        <div className="min-h-screen bg-[#0f172a] flex flex-col items-center text-white px-4">
            <div className="mb-6 text-center max-w-2xl">
                <h2 className="text-5xl font-semibold mb-4 mt-8">How to Use:</h2>
            ...
            <div className="border border-gray-400 rounded-2xl p-10 w-full max-w-xl bg-[#1e293b]">
                ...
                <div className="flex justify-around gap-4 flex-wrap mb-6">
                    <button onClick={() => handleSimulate('FCFS')} className="bg-red-500 hover:bg-red-600 px-4 py-2 rounded font-semibold cursor-pointer">FCFS</button>
                ...
                {result && (
                    <div className="bg-gray-800 p-4 rounded text-center text-gray-100">
                        {result}
                    </div>
                )}
            </div>
